...
addToSelectedWishlist:(value)=>set((state)=>{
    const wishlist= state.all_wishlists.find((list)=>list.listName===state.selectedWishlist)
    wishlist?.listItems.push(value)
    return {all_wishlists:[...state.all_wishlists]}
}),
removeItemFromWishlist:(listname,itemname)=>set((state)=>{
   const updatedWishList=state.all_wishlists.map((wishlist)=>{
    if(wishlist.listName===listname){
        const updatedItems= wishlist.listItems.filter((i)=>i.title!==itemname)
...
setAllWishlists:(item)=>set((state)=>({all_wishlists:[...state.all_wishlists,item]})),
onDeleteWishlist:(listname:string)=>set((state)=>({all_wishlists:state.all_wishlists.filter((item)=>item.listName!==listname)})),
isOpen:false,
